...
export async function swapHistory(account, pageNum = 1, showNum = 10, chainID) {
  ...
  const pair_addresses = [
    ...
    '0xBE460002a4198Ac842dfd98313eae2e979322C2e'
...
export async function rewardMine(account, pageNum = 1, showNum = 10) {
  const category = 'uniswap_stake,mlamb_stake';
  try {
    const data = await axios.get(`${baseUrl}api/txs?&category=${category}&from=${account}&pageNum=${pageNum}&showNum=${showNum}`);
    // console.log({ data });
    return data.data;
  } catch (error) {
    console.log(error);
